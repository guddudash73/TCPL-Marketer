...
export class ConfigurationService {
  ...
  async updateSector(
    ...
    return this.withUniqueConflict("Sector slug already exists", async () => {
      ...
      const sector = await this.database.client.sector.update({
        where: { id },
        data,
      });
      await this.record(
        "configuration.sector.updated",
        "sector",
        sector.id,
        actorUserId,
      );
  ...
  async createCapability(input: CreateCapabilityInput, actorUserId: string) {
    ...
    return this.withUniqueConflict(
      "Capability slug already exists in this sector",
      async () => {
        ...
        await this.record(
          "configuration.capability.created",
          "service_capability",
          capability.id,
          actorUserId,
        );
  ...
  async updateCapability(
    ...
    return this.withUniqueConflict(
      ...
      async () => {
        ...
        const capability = await this.database.client.serviceCapability.update({
          where: { id },
          data,
        });
        await this.record(
          "configuration.capability.updated",
          "service_capability",
          capability.id,
          actorUserId,
        );
  ...
  async deleteCapability(id: string, actorUserId: string) {
    await this.ensureCapability(id);
    const capability = await this.database.client.serviceCapability.delete({
      where: { id },
    });
    await this.record(
      "configuration.capability.deleted",
      "service_capability",
      capability.id,
      actorUserId,
    );
  ...
  async getDeliverable(id: string) {
    const deliverable =
      await this.database.client.capabilityDeliverable.findUnique({
        where: { id },
        include: { capability: true },
      });
  ...
  async createDeliverable(input: CreateDeliverableInput, actorUserId: string) {
    ...
    return this.withUniqueConflict(
      "Deliverable slug already exists for this capability",
      async () => {
        const deliverable =
          await this.database.client.capabilityDeliverable.create({
            data: { ...input, slug: input.slug ?? slugify(input.name) },
          });
        await this.record(
          "configuration.deliverable.created",
          "capability_deliverable",
          deliverable.id,
          actorUserId,
        );
        return deliverable;
  ...
  async updateDeliverable(
    ...
    return this.withUniqueConflict(
      ...
      async () => {
        const deliverable =
          await this.database.client.capabilityDeliverable.update({
            where: { id },
            data: input,
          });
        await this.record(
          "configuration.deliverable.updated",
          "capability_deliverable",
          deliverable.id,
          actorUserId,
        );
  ...
  async deleteDeliverable(id: string, actorUserId: string) {
    await this.ensureDeliverable(id);
    const deliverable = await this.database.client.capabilityDeliverable.delete(
      { where: { id } },
    );
    await this.record(
      "configuration.deliverable.deleted",
      "capability_deliverable",
      deliverable.id,
      actorUserId,
    );
  ...
  async createTargetProfile(
    ...
    await this.ensureCapability(input.capabilityId);
    return this.withUniqueConflict(
      "Target-client profile slug already exists for this capability",
      async () => {
        ...
        await this.record(
          "configuration.target_profile.created",
  ...
  async updateTargetProfile(
    ...
    return this.withUniqueConflict(
      ...
      async () => {
        ...
        const profile = await this.database.client.targetClientProfile.update({
          where: { id },
          data,
        });
        await this.record(
          "configuration.target_profile.updated",
          "target_client_profile",
          profile.id,
          actorUserId,
        );
  ...
  async deleteTargetProfile(id: string, actorUserId: string) {
    await this.ensureTargetProfile(id);
    const profile = await this.database.client.targetClientProfile.delete({
      where: { id },
    });
    await this.record(
      "configuration.target_profile.deleted",
      "target_client_profile",
      profile.id,
      actorUserId,
    );
  ...
  async createDecisionMaker(
    ...
    await this.ensureCapability(input.capabilityId);
    return this.withUniqueConflict(
      "Decision-maker title already exists for this capability",
      async () => {
        const profile = await this.database.client.decisionMakerProfile.create({
          data: input,
        });
        await this.record(
          "configuration.decision_maker.created",
  ...
  async updateDecisionMaker(
    ...
    return this.withUniqueConflict(
      "Decision-maker title already exists for this capability",
      async () => {
        const profile = await this.database.client.decisionMakerProfile.update({
          where: { id },
          data: input,
        });
        await this.record(
          "configuration.decision_maker.updated",
          "decision_maker_profile",
          profile.id,
          actorUserId,
        );
  ...
  async deleteDecisionMaker(id: string, actorUserId: string) {
    await this.ensureDecisionMaker(id);
    const profile = await this.database.client.decisionMakerProfile.delete({
      where: { id },
    });
    await this.record(
      "configuration.decision_maker.deleted",
      "decision_maker_profile",
      profile.id,
      actorUserId,
    );
  ...
  private async ensureSector(id: string): Promise<void> {
    this.requireRecord(
      await this.database.client.sector.findUnique({
        where: { id },
        select: { id: true },
      }),
      "Sector",
  ...
  private async ensureCapability(id: string): Promise<void> {
    this.requireRecord(
      await this.database.client.serviceCapability.findUnique({
        where: { id },
        select: { id: true },
      }),
      "Capability",
  ...
  private async ensureTargetProfile(id: string): Promise<void> {
    this.requireRecord(
      await this.database.client.targetClientProfile.findUnique({
        where: { id },
        select: { id: true },
      }),
  ...
  private async ensureDecisionMaker(id: string): Promise<void> {
    this.requireRecord(
      await this.database.client.decisionMakerProfile.findUnique({
        where: { id },
        select: { id: true },
      }),
